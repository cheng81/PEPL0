...
Response.prototype.perform = function(env,proc,rt) {
	if(this.isAuto==true) {
		...
		var executed = []
		var next = function() {
			var parents = s.path.eval(env,proc,rt)
			if(parents.length>0) {
				var def = parents[0].def.components[s.id]
				var prms = s.resolveParams(def.params,s.params,env,proc,rt)
				var p1 = wu(parents).filter(function(parent) {
					return !wu(executed).has(parent.uuid)
				}).toArray()
				if(p1.length>0) {
					var p = p1.shift()
					executed.push(p.uuid)
					//rt.execute(p.uuid,def.id,prms)
					if(p1.length>0) {
						rt.queue(next)
					}
					prms.__meta = {auto:true}
					return {procId:p.uuid,evtId:def.id,data:prms}
				}
...
Include.prototype.perform = function(env,proc,rt) {
	...
	if(parents.length>0) {
		if(this.params.length==0) {
			wu(parents).each(function(parent) {
				parent.state.included[this.id].all(proc,rt)
			},this)
		} else {	//TODO: remove possibility of partially include/exclude
			var def = parents[0].def.components[this.id]
			var prms = this.resolveParams(def.params,this.params,env,proc,rt)
			wu(parents).each(function(parent) {
...
Let.prototype.perform = function(env,procInst,runtime) {
	env.define(this.id,this.expr.eval(env,procInst,runtime))
}
